...
INCLUDES	    :=	inc
ROMFS	        :=  assets/romfs
APP_TITLE       :=  "Memory 3DS"
APP_DESCRIPTION :=  "A 3DS memory game!"
APP_AUTHOR      :=  "Omegadrien"
APP_PRODUCT_CODE:=  CTR-HB-MEMO
APP_UNIQUE_ID   :=  0x60AA1
...
MAKEROM := $(CURDIR)/makerom64.exe
BANNERTOOL := $(CURDIR)/bannertool64.exe
...
clean:
	@echo clean ...
	@rm -rf $(BUILD) $(OUTPUT).3dsx $(OUTPUT).smdh $(OUTPUT).elf $(OUTPUT).cia $(OUTPUT)_stripped.elf
